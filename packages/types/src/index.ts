...
export interface MCPTool {
  ...
  metadata?: { category?: string; tags?: string[]; deprecated?: boolean; requiresAuth?: boolean };
}
...
export interface MemoryConfig {
  namespace: string;
  hnsw?: { enabled: boolean; efConstruction?: number; M?: number };
  consolidation?: { auto: boolean; minAgeHours: number; maxEntries: number };
}

export interface AlignmentConfig {
  enabled: boolean;
  constraints: string[];
  circuitBreaker: { warnThreshold: number; throttleThreshold: number; shutdownThreshold: number };
}

export interface SybiothConfig {
  version: string;
  enhancementLevel: 'minimal' | 'standard' | 'full';
  runtime: { port: number; daemon: boolean; logLevel: string };
  models: ModelConfig;
  ...
  agents: { directories: string[] };
  hooks: { directories: string[] };
  adapter: { system: AdapterSystem };
}
